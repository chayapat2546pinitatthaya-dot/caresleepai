// ป้องกัน API Key: กรุณาตั้งค่า API Key ของคุณเอง
// วิธี: แทนที่ค่า YOUR_API_KEY_HERE ด้านล่างนี้
const GEMINI_API_KEY = 'AIzaSyCVKfkAUM1rHngcaEWlwz2EieocReBfYAY';
let isApiKeyValid = false;
let isDemoMode = false;

// ตรวจสอบและเตรียม API Key
function initializeApiKey() {
    // หาก key ยังเป็นค่าเริ่มต้น
    if (GEMINI_API_KEY === 'YOUR_API_KEY_HERE' || !GEMINI_API_KEY || GEMINI_API_KEY.length < 20) {
        isDemoMode = true;
        console.warn('⚠️ API Key is not configured. Using Demo Mode with simulated responses.');
        return false;
    }
    isApiKeyValid = true;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize API Key and show status
    initializeApiKey();
    if (isDemoMode) {
        console.info('💡 To enable full AI features, set your Gemini API Key in script.js');
    }
    ...
    // แสดงข้อความประบาทหากใช้ Demo Mode
    if (isDemoMode && chatMessages && chatMessages.children.length > 0) {
        const demoNotice = document.createElement('div');
        demoNotice.className = 'message bot';
        demoNotice.style.marginBottom = '1.5rem';
        demoNotice.innerHTML = `
            <div class="bot-avatar-msg">
                <img src="logo.png" alt="CareSleep">
            </div>
            <div class="message-bubble" style="background: rgba(251, 146, 60, 0.15); border: 1px solid rgba(251, 146, 60, 0.3);">
                <strong>🔔 Notice:</strong> Currently in Demo Mode (API Key not configured). 
                <br>To enable AI responses: Set your Gemini API Key in script.js
            </div>
        `;
        // ใส่หลังจากข้อความต่อนสุดท้าย หรือท้ายสุดถ้ามี child มากกว่า 1
        if (chatMessages.children.length > 1) {
            chatMessages.insertBefore(demoNotice, chatMessages.children[1]);
        } else {
    ...
    async function getGeminiResponse(userPrompt) {
        // หากไม่ได้ configure API Key
        if (!isApiKeyValid || isDemoMode) {
        ...
        กฎสำคัญ:
        - ห้ามบอกว่าคุณคือ AI ของ Google, Gemini หรือโปรแกรมคอมพิวเตอร์เด็ดขาด คุณคือ "ลูน่า" โค้ชตัวจริงเท่านั้น
        - หากถูกถามว่าเป็น AI อะไร หรือใช้เทคโนโลยีอะไร ให้ตอบว่า "ฉันคือลูน่า โค้ชส่วนตัวที่คุณไว้วางใจในการดูแลการนอนค่ะ"
        ...
        - ให้คุณทำหน้าที่เป็นนักวิเคราะห์ โดยการถามคำถามเจาะลึกเพิ่ม 2-3 ข้อ เพื่อหาสาเหตุ เช่น:
            1. สถาพแวดล้อม: อุณหภูมิห้อง, แสงไฟ, หรือเสียง rรบกวนเป็นอย่างไร?
        ...
        // รายชื่อโมเดลที่รองรับ (เรียงตามลำดับความเสถียรและความพร้อมใช้งาน)
        ...
        for (const model of models) {
            try {
                ...
                // ตรวจสอบข้อผิดพลาด 429 (Rate Limit / Quota)
                if (resp.status === 429) {
                    throw new Error('Quota Exceeded (429): ไม่พร้อมใช้งานชั่วคราว (ลองใหม่ในอีก 1-2 นาทีนะครับ)');
                }

                // ตรวจสอบข้อผิดพลาด API อื่นๆ
                if (resp.status === 401 || resp.status === 403) {
                    throw new Error('API Key invalid or suspended. Please check your API key.');
                }
                ...
                // ถ้าเป็นปัญหาโควตา ให้หยุดลองโมเดลอื่นทันทีเพราะโควตามักจะใช้ร่วมกัน
                if (lastError.includes('Quota')) break;
                console.warn(`Error with ${model}:`, err);
            }
        }
        throw new Error(lastError || 'Failed to get response from API');
    }
    ...
    async function handleChat() {
        ...
        try {
            ...
            // ใช้ API หาก API Key ถูกตั้งค่าถูกต้อง
            if (isApiKeyValid && !isDemoMode) {
                response = await getGeminiResponse(text);
            } else {
                // Demo Mode: ใช้ response ที่กำหนดไว้
                await new Promise(r => setTimeout(r, 800));

                // เลือก response แบบสุ่ม
                const responses = translations[currentLang].bot_responses;
                response = responses[Math.floor(Math.random() * responses.length)];

                // เพิ่มป้ายชี้ว่ากำลังใช้ Demo Mode
                if (isDemoMode) {
                    response = `[📌 Demo Mode]\n\n${response}`;
                }
            }
